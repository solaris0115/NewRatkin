    ...
    public class Projectile_BallistaBoltAP : Projectile
    {
        ...
        public new void Launch(Thing launcher, Vector3 origin, LocalTargetInfo usedTarget, LocalTargetInfo intendedTarget, ProjectileHitFlags hitFlags, Thing equipment = null, ThingDef targetCoverDef = null)
        {
            ...
            if (equipment != null)
            {
                ...
                weaponDamageMultiplier = equipment.GetStatValue(StatDefOf.RangedWeapon_DamageMultiplier, true);
            }
            else
            {
                equipmentDef = null;
        ...
        private bool CheckForFreeIntercept(IntVec3 c)
        {
            ...
            for (int i = 0; i < thingList.Count; i++)
            {
                ...
                //Log.Message(thing.Label + "CanHit : " + CanHit(thing));
                if (CanHit(thing))
                {
                    bool flag2 = false;
                    ...
                    num2 *= num;
                    //Log.Message("HitChance : " + thing.Label+ " / " +  num2 );
                    if (num2 > 1E-05f)
                    {
                        if (Rand.Chance(num2))
                        {
